...
# Script to install and configure dev tools for all popular languages and editors on Ubuntu/Debian/Raspberry Pi OS (arm64)
...
# Install databases (use mariadb-server for ARM compatibility)
sudo DEBIAN_FRONTEND=noninteractive apt install -y postgresql mariadb-server sqlite3

# Install editors
sudo apt install -y vim emacs nano

# Install VSCodium for arm64
wget https://github.com/VSCodium/vscodium/releases/download/1.107.18627/codium_1.107.18627_arm64.deb
sudo dpkg -i codium_1.107.18627_arm64.deb
rm codium_1.107.18627_arm64.deb
